...
Пакет `hh-vacancies` использует публичный программный интерфейс `api.hh.ru` для трёх сценариев только для чтения:
...
Если `node -e 'require("hh-vacancies")'` не проходит, сначала ставится пакет, а не пишутся разовые запросы к программному интерфейсу HH.
...
- Источник явно назван публичным программным интерфейсом `hh.ru` только для чтения
