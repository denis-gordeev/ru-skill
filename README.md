...
- Верхнеуровневая документация и дорожная карта переведены на единый русскоязычный сценарий с явным разделением `Target` и `Legacy`.
...
- Для setup и shell-скриптов введён двойной путь secrets: сначала `~/.config/ru-skill/secrets.env`, затем устаревший запасной вариант `~/.config/k-skill/secrets.env`.
...
- В `docs/roadmap.md` обновлён статус Вехи 4: документная миграция, матрица замен и маркировка устаревших пакетов доведены до рабочего завершения; дальше фокус смещён на booking-replacements и релиз-гигиену.
- Добавлена отдельная doc-regression проверка, которая не даёт README, дорожную карту и TODO разъехаться по следующим шагам и продуктовым приоритетам.
- Проведён раунд релиз-гигиены: подтверждён текущий inventory `.changeset/`, а из верхнеуровневых документов убраны устаревшие релизные ярлыки и сводки расстояния ветки как неустойчивый live-статус.
- Doc-regression усилен: README и дорожная карта теперь дополнительно страхуются тестами от возврата устаревшей релиз-археологии в живые секции.
...
- Доведена до конца `remaining устаревший без развития matrix`: `seoul-subway-arrival` и `toss-securities` теперь одинаково помечены как документно закрытые `Legacy`, а `k-skill-proxy` — как `Transition` во всех user-facing верхнеуровневых документах.
- `docs/install.md` больше не смешивает `target`, `устаревший без развития` и переходные навыки без пояснений: в install-flow зафиксированы границы для `delivery-tracking`, `seoul-subway-arrival`, `toss-securities` и `k-skill-proxy`.
- User-facing guides для `delivery-tracking`, `seoul-subway-arrival`, `toss-securities` и `k-skill-proxy` дополнены явными граничными примечаниями, чтобы устаревшие/переходные сценарии не выглядели как скрытый целевой перечень задач.
- `fine-dust-location`, `docs/setup.md`, `docs/security-and-secrets.md` и `packages/k-skill-proxy/README.md` выровнены по `ru-skill`-first порядку секретов и границе перехода: опубликованный прокси-эндпоинт описан как слой совместимости, а `AIR_KOREA_OPEN_API_KEY` оставлен только для прямого резервного доступа или прокси на собственном сервере.
- Doc-regression расширен на `fine-dust-location` и прокси вспомогательных документов, чтобы граничные примечания, порядок `~/.config/ru-skill/secrets.env` -> `~/.config/k-skill/secrets.env` и различие между переопределением адреса и реальными секретами не разъезжались.
...
- Doc-regression расширен ещё на secrets template и вспомогательных документов настройки, чтобы необязательное переопределение/настоящие учётные данные не откатывалась в локальных инструкциях и проверках окружения.
- Package-level README для устаревших вспомогательных пакетов (`toss-securities`, `daiso-product-search`, `kleague-results`, `blue-ribbon-nearby`, `kakao-bar-nearby`, `k-lotto`) выровнены с текущим управлением миграцией: они теперь явно фиксируют `устаревший без развития` границу, называют российские replacements там, где они уже есть, и не выглядят как активный целевой перечень задач.
- Doc-regression расширен на этот package-level слой, чтобы `устаревший без развития` граница удерживалась не только в feature guides и top-level docs, но и в README отдельных workspace-пакетов.
- Устаревший псевдоним `k-skill-setup` больше не продвигает `~/.config/k-skill/bin` и `~/.config/k-skill/logs` как значение по умолчанию для выполнения: update-check examples и log paths переведены на `~/.config/ru-skill/*`, а устаревшие пути оставлены только как совместимый запасной вариант.
- Doc-regression расширен на setup runtime-artifacts, чтобы даже устаревший навык настройки не возвращал `k-skill`-prefixed bin/log directories в роли основного рабочего пути.
- Оставшиеся устаревшие guides уровня навыков тоже выровнены с границей миграции: `fine-dust-location`, `srt-booking` и `ktx-booking` теперь одинаково фиксируют устаревшую/переходную роль, `ru-skill`-first secrets order и различие между необязательным переопределением адреса и реальными секретами.
- Doc-regression расширен на этот skill-level слой, чтобы railway/fine-dust copy не возвращала устаревшие значения по умолчанию или скрытый целевой перечень задач в новые раунды.
- Следующий слой устаревших guides уровня навыков тоже выровнен: `kakaotalk-mac`, `kbo-results`, `lotto-results` и `zipcode-search` теперь явно фиксируют `устаревший без развития` границу и не выглядят как активные целевые кандидаты.
- Doc-regression расширен и на эти skill-only surfaces, чтобы граница миграции для локальных CLI и корейских вспомогательных сценариев не терялся между `docs/features/*` и `*/SKILL.md`.
- Следующий слой устаревшего feature/skill дрейфа тоже закрыт: `blue-ribbon-nearby`, `daiso-product-search`, `kakao-bar-nearby`, `kleague-results`, `srt-booking` и `ktx-booking` теперь в user-facing guides явно публикуют `## Граничное примечание`, а их границы замен синхронизированы со связанными `SKILL.md`.
- Doc-regression расширен и на этот feature/skill слой, чтобы replacement copy для nearby, marketplace, football и устаревших железнодорожных сценариев не расходился между `docs/features/*` и `*/SKILL.md`.
- Вспомогательная/runtime-очистка формально закрыта и на user-facing устаревших guides: `fine-dust-location`, `seoul-subway-arrival`, `srt-booking` и `ktx-booking` теперь дополнительно страхуются регрессиями на `ru-skill`-first secrets order, runtime/secrets semantics и граница замен.
...
- Doc-regression расширен на все 13 целевых навыков (ранее непокрытые: `moex-shares`, `stoloto-lotto`, `kinopoisk-search`, `pravo-documents`, `rpl-results`, `osm-nearby`).
- `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы: оставшийся Korean в source code и docs - domain-inherent (API parameters, location names, эталонные данные), следующий приоритет - расширить doc-regression охват для устаревших навыков.
...
- Doc-regression теперь покрывает все 13 целевых навыков и 4 устаревших навыка с проверками сценария и содержимого (`seoul-subway-arrival`, `kbo-results`, `lotto-results`, `srt-booking`).
- Английские заголовки секций во всех целевых SKILL.md переведены на русский и приведены к единой схеме; нестандартные русские формулировки в `yandex-rasp` и `yandex-market-search` нормализованы; последние Korean фрагменты в feature docs (`kakao-bar-nearby`, `kleague-results`) переведены на русский.
- `ru-skill-setup/SKILL.md` переведён на русские секционные заголовки (`Назначение`, `Порядок разрешения учётных данных`, `Стандартный сценарий`, `Совместимость`), чтобы предпочтительный псевдоним настройки не оставался последним English-heading outlier.
- Устаревший псевдоним `k-skill-setup/SKILL.md` теперь использует ту же верхнеуровневую heading scheme (`Назначение`, `Порядок разрешения учётных данных`, `Стандартный сценарий`, `Совместимость`) без изменения потока настройки, порядка секретов и семантики совместимости.
...
- `docs/features/zoon-nearby.md` и `packages/zoon-nearby/README.md` очищены от смешанного артефакта `可以直接`; doc-regression теперь дополнительно страхует heading scheme для псевдонима настройки и `zoon-nearby`, а также отсутствие такого смешанноязычный дрейф.
...
- Doc-regression расширен на предпочтительный навык настройки и гигиену живого backlog: тесты теперь страхуют русские заголовки в `ru-skill-setup` и то, что активные unchecked-пункты живут только в верхнем plan block `TODO.md`.
...
- Doc-regression дополнительно страхует этот слой: `scripts/skill-docs.test.js` проверяет новые русские формулировки и не даёт затронутых устаревших поверхностей вернуть старую английскую граничную/продуктовую копию.
...
- Все 20 workspace `package.json` descriptions переведены на русский и выровнены с текущим целевой/устаревший/переходный позиционированием репозитория, чтобы npm/publish-метаданные не расходилась с README и feature docs.
- Doc-regression расширен на переходные поверхностей настройки и метаданные пакетов: тесты теперь страхуют русские формулировки вокруг `KSKILL_PROXY_BASE_URL`, `AIR_KOREA_OPEN_API_KEY` и descriptions во всех publishable workspace-пакетах.
- Полный `npm test` и `./scripts/validate-skills.sh` проходят после этой синхронизации.
- Закрыт следующий мелкий слой смешанноязычный дрейф в repo-governance и поверхностях публикации: `packages/zoon-nearby/README.md` переведён с неканоничного `## Обзор` на `## Что делает навык`, `AGENTS.md` русифицирован, а оставшиеся формулировки на основе эталонных данных в `.changeset/fair-steaks-pretend.md` и `.changeset/moex-shares.md` переведены на русский.
- Doc-regression расширен на package README heading и поверхностях управления репозиторием: тесты страхуют, что `packages/zoon-nearby/README.md` не возвращает `## Обзор`, `AGENTS.md` не возвращает `Default posture: public read-only endpoint`, а формулировки на основе эталонных данных не возвращаются в changeset-сводках.
- Закрыт следующий слой English jargon в shell/infrastructure поверхностях: `scripts/check-setup.sh` и `scripts/validate-skills.sh` теперь выдают русские user-facing статусы, ошибки и подсказки вместо `missing`, `insecure`, `next steps`, `skill layout looks valid`.
- Doc-regression расширен на shell/infrastructure поверхностях: тесты страхуют русские сообщения в `scripts/check-setup.sh` и `scripts/validate-skills.sh`, а также не дают вернуть английские вспомогательные/status-формулировки в эти CLI-скрипты.
...
- Skill-only drift, устаревший feature/skill дрейф, вспомогательная/runtime-очистка, source-level русификация и skill-level copy audit уже закрыты; весь user-facing Korean в source code и docs полностью устранён; оставшийся Korean — domain-inherent (API parameters, location names, эталонные данные, regex patterns).
...
- Package README на актуальных целевых поверхностях тоже доведены до каноничной heading scheme; оставшийся `## Обзор` в `packages/zoon-nearby/README.md` устранён.
...
- Doc-regression покрывает все 13 целевых навыков и все устаревшие навыки с проверками сценария и содержимого, каноничность heading scheme и отсутствие mixed-language артефактов.
...
- На обновлённых устаревших README пакетов и feature guides снят следующий слой user-facing English drift; следующий проход нужен уже уже по переходным поверхностям и метаданные пакетов, а не по тем же документам с граничными примечаниями.
- Переходные поверхности настройки и метаданные пакетов descriptions теперь тоже выровнены; следующий проход нужен уже по публикации/релиз-поверхностям (`CHANGELOG.md`, changeset summaries, feature-link labels, skill frontmatter descriptions), а не по тем же документам учётных данных/прокси.
- Все 7 английских changeset-сводок в `.changeset/` переведены на русский, чтобы publish-summary copy не отставала от README и метаданных пакетов.
...
- Doc-regression расширен на changeset summaries и SKILL.md frontmatter descriptions: тесты теперь страхуют русские формулировки на этих поверхностях публикации.
...
- Shell/infrastructure поверхности больше не выбиваются по языку: `scripts/check-setup.sh` и `scripts/validate-skills.sh` синхронизированы с русскоязычным setup/runtime контуром и застрахованы doc-regression тестами.
- Source-code error messages и вспомогательные сообщения Python в устаревших пакетах (`toss-securities`, `kleague-results`, `kakao-bar-nearby`, `blue-ribbon-nearby`, `k-lotto`, `daiso-product-search`, `ktx_booking.py`, `fine_dust.py`) переведены на русский и застрахованы doc-regression тестами.
...
- Следующий проход делать уже по оставшимся поверхностям крайних случаев: npm script output, вспомогательные JS-утилиты (`fix-changelog-headings.js`), и другим редким repo-infrastructure сообщениям.
