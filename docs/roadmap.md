...
Статус: в работе; release-hygiene подзадача закрыта, остаётся booking-research и решение по replacement boundary.
...
3. Держать в CI синхрон верхнеуровневой документации не только по install-flow, но и по живому плану следующего продуктового шага, а также по отсутствию устаревшей release-археологии в README/roadmap.
4. Собрать decision matrix для booking replacement: официальный поток РЖД, агрегаторный read-only сценарий и fallback-вариант с handoff без автоматизации оплаты.
