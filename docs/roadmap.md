...
Статус: в работе; release-hygiene подзадача закрыта, booking-research проведён документно, остаётся решение нужен ли отдельный read-only/handoff skill сверх `yandex-rasp`.
