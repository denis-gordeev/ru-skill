...
Статус: в работе (перевод SKILL.md завершён, матрица замен обновлена, реализован 10-й target-навык `rpl-results`).
...
1. Реализован 10-й российский target-навык `rpl-results` для турнирных таблиц и результатов матчей РПЛ (пакет создан, fixture-тесты добавлены).
2. Продолжить поэтапную матрицу замен legacy-пакетов российскими аналогами в рамках Milestone 4.
3. Исследовать публичные HTML-поверхности для marketplace-поиска (Яндекс.Маркат, Price.ru, E-katalog) без requirements к API-ключам.
4. Решить, нужен ли аналогичный alias-layer для proxy-сценария, или достаточно legacy-маркировки `k-skill-proxy`.
5. Подготовить 11-й и 12-й российские навыки (маркетплейс и nearby-поиск) после нахождения стабильных public surfaces.
