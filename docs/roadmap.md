...
1. Выбрать восьмой российский read-only источник, чтобы target-линейка продолжала расти после финансов, лотерей, почтовых индексов, рынка труда, кино и публичной безопасности.
2. Решить, нужен ли отдельный region-lookup helper для `mchs-storm-warnings`, чтобы пользователь мог искать регион по названию, а не только по хосту.
3. Перейти от наращивания target-пакетов к явной матрице замещения legacy-пакетов в рамках Milestone 4.
