...
Статус: в работе (перевод SKILL.md завершён, матрица замен обновлена, реализованы 10-й и 11-й target-навыки `rpl-results` и `yandex-market-search`).
...
5. Решить, нужен ли аналогичный alias-layer для proxy-сценария, или достаточно legacy-маркировки `k-skill-proxy`.
