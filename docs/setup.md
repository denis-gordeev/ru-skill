...
| `fine-dust-location` | Обычно ничего: опубликованный совместимый прокси-конечная точка используется по умолчанию. Для переопределения нужен `KSKILL_PROXY_BASE_URL`, для прямого резервного доступа или прокси на собственном сервере - `AIR_KOREA_OPEN_API_KEY`. |
