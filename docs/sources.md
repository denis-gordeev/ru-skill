...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных тесты на JSON-нормализацию ISS.
...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных тесты на HTML-нормализацию страниц `cities` и `offices`.
...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных JSON-тесты.
...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных тесты на HTML-нормализацию страниц архива.
...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных тесты на HTML-нормализацию.
...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных HTML-тесты для списка предупреждений и одной карточки.
...
- Техническая основа: навык + пакет только для чтения + на основе эталонных данных JSON-тесты на ответы API.
...
- championat.com использует серверно отрендеренный HTML, который можно надёжно парсить без JS-рендеринга.
...
- На текущем этапе поисковая выдача и карточки товаров доступны как серверно отрендеренный HTML, который можно нормализовать без браузерной автоматизации и без API-ключа.
...
- Это бесплатный публичный API без необходимости API-ключа или авторизации.
...
- Price.ru и E-katalog остаются резервными кандидатами, если текущий серверно отрендеренный интерфейс (SSR) Яндекс Маркета перестанет быть доступной.
- **Решение**: основная замена для `daiso-product-search` уже закрыта через `yandex-market-search`; резервный запасной вариант держать на случай деградации серверно отрендеренных страниц (SSR) Маркета.

#### Поиск ближайших (2GIS/Яндекс.Карты/Zoon)

- 2GIS Catalog API требует API-ключ (docs.2gis.com).
- Яндекс.Карты Geosearch API требует API-ключ.
- Zoon.ru — публичный справочник организаций, серверно отрендеренные страницы (SSR), без антибота, без API-ключей.
