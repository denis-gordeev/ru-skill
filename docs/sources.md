...
- Для target-линейки безопаснее считать railway replacement read-only/handoff задачей, а не новым checkout automation проектом.
- На 2026-04-29 отдельный handoff-skill не открывается: текущую пользовательскую границу достаточно закрывают `yandex-rasp` и ручной переход пользователя в существующие checkout-потоки.
