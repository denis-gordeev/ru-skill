...
Если нужен почти прямой доступ к AirKorea, можно использовать конечную точку сквозного маршрута. При этом посредник сам подставляет `serviceKey`, а отдельный клиентский API не нужен.
...
- При развёртывании на сервере ключ вышестоящего API AirKorea должен оставаться только на посреднике, а не на клиенте.
