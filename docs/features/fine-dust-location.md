...
- Либо `k-skill-proxy`, либо ключ открытого программного интерфейса Air Korea

## Нужные переменные окружения

Предпочтительный клиентский режим:

- Внешний адрес посредника по умолчанию: `https://k-skill-proxy.nomadamas.org`
- `KSKILL_PROXY_BASE_URL` задаётся только если нужно переопределить эту конечную точку
- Отдельный клиентский ключ программного интерфейса в этом режиме не нужен

Только для прямой резервный доступ без посредника или для собственного сервера-посредника:
...
1. Если задан `KSKILL_PROXY_BASE_URL`, сначала вызвать `/v1/fine-dust/report` на этом посреднике; если переменная не задана, использовать опубликованный совместимую конечную точку `https://k-skill-proxy.nomadamas.org`.
2. Если пришёл `regionHint`, посредник сначала выделяет название региона и получает список станций через `getCtprvnRltmMesureDnsty`.
3. Если маркер из региона однозначно соответствует одной станции, посредник вызывает `getMsrstnAcctoRltmMesureDnsty` для неё.
4. Если однозначности нет, посредник возвращает `ambiguous_location` и `candidate_stations`.
...
Пример через посредника:

```bash
python3 scripts/fine_dust.py report --region-hint "서울 강남구" --json
```

Пример, когда посредник возвращает кандидатов:
...
Если нужен почти прямой доступ к AirKorea, можно использовать конечную точку сквозного маршрута. При этом посредник сам подставляет `serviceKey`, а отдельный клиентский программный интерфейс не нужен.
...
- Даже если программный интерфейс `station-list` отдаёт `403`, можно обойтись комбинацией `getCtprvnRltmMesureDnsty` и измерений по станции.
...
- Если программный интерфейс не прислал `khaiGrade`, интегральный уровень нужно выводить как «Нет данных».
- `regionHint` описывает место в естественном языке, поэтому неоднозначность там частая.
- При развёртывании на сервере ключ вышестоящего программного интерфейса AirKorea должен оставаться только на посреднике, а не на клиенте.
- Публичный посредник и устаревшее именование здесь остаются слоем совместимости, а не рекомендацией расширять `ru-skill` новыми корейскими сценариями источников данных.
