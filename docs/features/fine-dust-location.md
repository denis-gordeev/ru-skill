...
- Отдельный клиентский API-ключ в этом режиме не нужен
...
3. Если env нет, сначала искать `~/.config/ru-skill/secrets.env`, затем устаревший резерв `~/.config/k-skill/secrets.env`.
...
1. Если задан `KSKILL_PROXY_BASE_URL`, сначала вызвать `/v1/fine-dust/report` на этом прокси; если переменная не задана, использовать опубликованный совместимый эндпоинт `https://k-skill-proxy.nomadamas.org`.
