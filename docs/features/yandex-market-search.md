...
`yandex-market-search` — это клиент только для чтения для поиска товаров и чтения карточек товаров на публичных серверно отрендеренных страницах Яндекс Маркета. Он нужен как российская замена legacy-навыка `daiso-product-search`.
...
- `url` должен быть canonical `/card/{slug}/{id}` без отслеживающих параметров запроса
...
- Парсер опирается на текущую серверно отрендеренную вёрстку (SSR). При сильном изменении шаблонов нужен новый раунд эталонных данных
