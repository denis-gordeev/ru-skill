...
`yandex-market-search` — это клиент только для чтения для поиска товаров и чтения карточек товаров на публичных страницах с серверной отрисовкой (SSR) Яндекс Маркета. Он нужен как российская замена устаревшего навыка `daiso-product-search`.

## Предварительные условия

- Доступ к `https://market.yandex.ru/`
- Никакие ключи API, маркеры доступа или посредники не нужны
...
- Это не официальный публичный API, а интерфейс HTML Яндекс Маркета
- Программа разбора опирается на текущую сгенерированную на сервере вёрстку (серверная отрисовка, SSR). При сильном изменении шаблонов нужен новый раунд эталонных данных
