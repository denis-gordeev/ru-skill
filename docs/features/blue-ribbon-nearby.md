...
- Выполнять поиск ближайших по координатам.
...
Ориентиры через внутренние псевдонимы приводятся к ближайшей официальной зоне Blue Ribbon. Например, `코엑스` -> `삼성동/대치동`.
Устаревшая формулировка для регрессий: поиск ресторанов рядом по умолчанию через blue-ribbon-nearby.
...
Ключевые параметры запроса поиска ближайших:
...
Если есть только координаты, область поиска ближайших строится через ограничивающую рамку на `latitude1`, `latitude2`, `longitude1`, `longitude2`.

## Рабочий процесс

1. Сначала спросить текущее местоположение.
2. Если пользователь прислал район, станцию или ориентир, найти ближайшую официальную зону через `search/zone`.
3. Если пользователь прислал координаты, посчитать ограничивающую рамку для поиска ближайших.
...
- Blue Ribbon может отдавать `403`, если запрос недостаточно похож на запрос из обозревателя.
- Если структура списка зон изменится, поменяется и логика сопоставления.
