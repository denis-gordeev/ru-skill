...
Этот прокси остаётся `transition`-слоем, а не отдельным пользовательским target-навыком. Его стоит поднимать только под конкретные бесплатные public API-сценарии; само наличие `k-skill-proxy` в репозитории не означает, что legacy AirKorea-поток или другие корейские источники снова становятся направлением развития `ru-skill`.

Сейчас в коде уже реализован legacy-адаптер для AirKorea и сводка по fine dust. Это не основной будущий фокус репозитория, но хороший референс того, как в проекте должен выглядеть безопасный прокси только для чтения.
...
- Базовый режим: публичный endpoint только для чтения без proxy auth.
...
- Legacy endpoint `k-skill-proxy.nomadamas.org` сохраняется ради совместимости, но не должен выглядеть как основной пользовательский вход в новые target-сценарии.
