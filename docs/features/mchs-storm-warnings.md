...
`mchs-storm-warnings` - это обёртка только для чтения над официальными региональными страницами МЧС России для списка последних экстренных предупреждений и чтения карточки конкретного предупреждения.
...
- Нужно получить нормализованный текст предупреждения, дату публикации и ссылку на экспорт в формате PDF/Word страницы
...
2. Уточнить региональное имя узла МЧС, например `46`, `78`, `moscow` или название региона `"Курская область"`, `"Москва"`.
