...
- Парсинг HTML выполняется через извлечение регулярными выражениями, аналогично `stoloto-lotto` и `postcalc-postcodes`.
...
- Вёрстка страниц может меняться; тесты на основе эталонных данных помогают отлавливать регрессии парсинга.
