...
Навык `kinopoisk-search` позволяет получать информацию о фильмах через публичные страницы крупнейшей русскоязычной базы данных о кино — Кинопоиска. Это сценарий только для чтения, не требующий авторизации, секретов или прокси.
