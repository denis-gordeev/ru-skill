...
`pravo-documents` - это обёртка только для чтения над официальным API портала правовой информации для поиска федеральных законов, постановлений и других правовых актов с получением нормализованных метаданных документа.
...
2. Определить поисковый запрос или фильтры (тип документа, диапазон дат).
...
- Это сценарий только для чтения метаданных; содержимое PDF не скачивается и не разбирается
- Источник данных: официальный API `https://publication.pravo.gov.ru/api/`
- API не поддерживает полнотекстовый поиск с булевыми операторами
- Для сложных запросов может потребоваться комбинация фильтров
- `publishDateShort` приходит в формате ISO без указания часового пояса
