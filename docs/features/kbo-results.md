...
Этот сценарий остаётся `legacy-only`: для российских футбольных сводок его замена уже реализована как `rpl-results`. `kbo-results` сохраняется ради обратной совместимости и как эталонный сценарий для табло и результатов, но не должен выглядеть как активный backlog целевой линейки репозитория.
