...
Этот сценарий остаётся `legacy-only`: для российских футбольных сводок его замена уже реализована как `rpl-results`. `kbo-results` сохраняется ради обратной совместимости и как эталонный сценарий для табло и результатов, но не должен выглядеть как активный перечень задач целевой линейки репозитория.
...
- У `kbo-game@0.0.2` рабочий экспорт называется `getGame`; пример с `getGameInfo` из README не сработает.
