...
`hh-vacancies` - это обёртка только для чтения над публичным программным интерфейсом `api.hh.ru` для поиска вакансий, чтения карточки вакансии и поиска региона по `areaId`.

## Когда использовать

- Нужна русскоязычная или российская подборка вакансий по запросу и региону
- Нужно быстро открыть нормализованную карточку конкретной вакансии HH без ручной чистки HTML в обозревателе
...
- Источник данных: публичные конечные точки `https://api.hh.ru/vacancies`, `https://api.hh.ru/vacancies/{vacancy_id}` и `https://api.hh.ru/areas/{area_id}`
