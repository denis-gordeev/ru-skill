...
Этот навык остаётся `legacy-only`: прямой российский replacement для авторизованных брокерских сценариев не подтверждён, а публичные рыночные read-only сводки уже покрываются `moex-shares`. Поэтому `toss-securities` сохраняется для backward compatibility и не должен выглядеть как незавершённый target-кандидат.
