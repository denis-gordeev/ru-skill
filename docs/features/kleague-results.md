...
Этот сценарий остаётся `legacy-only`: для российских футбольных сводок его замена уже реализована как `rpl-results`. `kleague-results` сохраняется ради обратной совместимости и как эталонный сценарий для официальных расписаний и таблиц, но не должен подаваться как незавершённый перечень задач целевой линейки.
...
- Пока пакет новый, до релиза его стоит проверять через локальный workspace или `pack`-артефакт, а публикацию делать уже после слияния.
