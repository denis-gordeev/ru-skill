...
Здесь используется не разбор HTML, а данные в формате JSON и официальные программные интерфейсы.
...
- Пока пакет новый, до выпуска его стоит проверять через локальное рабочее пространство или архивный артефакт, а публикацию делать уже после слияния.
