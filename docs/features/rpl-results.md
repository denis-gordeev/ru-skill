...
- Для текущих матчей и онлайн-обновлений — данные обновляются после окончания матчей
...
Разбор происходит через регулярные выражения без зависимости от программ разбора объектной модели документа (`DOM`).
...
- Доступ к championat.com (без проверки подлинности)
- **Секреты не нужны**

## Ограничения и риски

- championat.com может изменить структуру HTML — тогда нужно обновить программу разбора
- Данные обновляются после окончания матчей, не в реальном времени
- championat.com — агрегатор, не официальный источник РПЛ
- Для официальных данных используйте premierliga.ru (требует исполнения `JavaScript`)
...
- [x] Данные разбираются с championat.com без проверки подлинности и секретов
