...
Дополнительные npm- или пакеты Python не нужны: достаточно официальных конечных точек.
...
2. Для CJ прочитать `_csrf` с официальной страницы и только потом вызвать конечную точку JSON `tracking-detail`.
