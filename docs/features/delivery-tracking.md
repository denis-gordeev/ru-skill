...
Дополнительные npm- или Python-пакеты не нужны: достаточно официальных конечных точек.
...
2. Для CJ прочитать `_csrf` с официальной страницы и только потом вызвать JSON-конечную точку `tracking-detail`.
...
5. При добавлении нового перевозчика придерживаться той же схемы полей модуля-посредника: `валидатор / точка входа / транспорт / программа разбора / таблица статусов / политика повторных попыток`.
