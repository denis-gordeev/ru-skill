...
5. При добавлении нового перевозчика придерживаться той же схемы полей модуля-посредника: `валидатор / точка входа / транспорт / парсер / таблица статусов / политика повторных попыток`.
