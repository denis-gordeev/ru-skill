...
Этот сценарий остаётся `legacy-only`: он полезен для обратной совместимости и для референса по adapter-based tracking flow, но не считается следующим российским target-направлением репозитория. Пока не подтверждён устойчивый российский public source с сопоставимой ценностью, `delivery-tracking` не должен выглядеть как скрытый backlog на новый replacement.
