...
- Держать единые правила адаптера перевозчика внутри одного навыка.

## Граничное примечание

Этот сценарий остаётся `устаревший без развития`: он полезен для обратной совместимости и как эталонный сценарий адаптерного отслеживания, но не считается следующим российским направлением целевой линейки репозитория. Пока не подтверждён устойчивый российский публичный источник с сопоставимой ценностью, `delivery-tracking` не должен выглядеть как скрытый перечень задач на новую замену.
...
Дополнительные npm- или Python-пакеты не нужны: достаточно официальных эндпоинтов.
...
1. Сначала проверить длину номера через валидатор конкретного перевозчика.
...
5. При добавлении нового перевозчика придерживаться той же схемы полей адаптера: `валидатор / точка входа / транспорт / парсер / таблица статусов / политика повторных попыток`.
