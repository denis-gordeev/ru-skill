...
- `k-skill-proxy` не является отдельным конечным пользовательским skill в этом потоке установки: это `transition`-инфраструктура для бесплатных API, которую имеет смысл поднимать только когда конкретный сценарий вроде `fine-dust-location` действительно требует прокси-слоя.
