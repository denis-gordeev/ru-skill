...
Установку не делим на отдельные ветки только из-за авторизации. Базовый подход такой: сначала ставится весь комплект, а подготовка секретов и переменных окружения передаётся `ru-skill-setup`. Устаревшее имя `k-skill-setup` остаётся совместимым псевдонимом.
