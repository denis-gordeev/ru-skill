...
| Публичный URL прокси | `k-skill-proxy.nomadamas.org` | Используется как опубликованный legacy endpoint | Оставить как legacy endpoint, новые русскоязычные adapter'ы описывать как `ru-skill` capabilities поверх этого слоя |
