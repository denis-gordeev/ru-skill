...
description: Поиск и чтение официальных правовых документов через программный интерфейс pravo.gov.ru без проверки подлинности. Использовать, когда пользователю нужно найти федеральные законы, постановления правительства или другие официальные правовые акты по названию, типу или дате.
license: MIT
metadata:
...
Пакет `pravo-documents` использует официальный программный интерфейс портала правовой информации для двух сценариев только для чтения:
...
- Программный интерфейс может вернуть пустой результат для редких запросов
- Некоторые старые документы могут не иметь версии SVG
- `publishDateShort` нормализуется из программного интерфейса и не гарантирует точное время публикации
