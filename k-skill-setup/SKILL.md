...
Выполняет общие послеустановочные действия после полной установки набора `ru-skill`.
...
- Опционально: проверка отметки звездой на GitHub с явного согласия
...
- Отметка звездой на GitHub выполняется только при явном согласии пользователя
...
2. **Если агент использует собственное хранилище секретов** (`1Password CLI`, `Bitwarden CLI` (интерфейсы командной строки), Связка ключей macOS и т.д.), извлечь оттуда и передать как переменную окружения.
3. **Сначала `~/.config/ru-skill/secrets.env`**, при отсутствии — устаревший резерв **`~/.config/k-skill/secrets.env`**. Оба файла предполагают формат простого файла окружения (dotenv) и права `0600`.
...
- предпочтительный файл секретов: `~/.config/ru-skill/secrets.env`
...
`KSKILL_PROXY_BASE_URL` намеренно не включён в этот минимальный шаблон: для `fine-dust-location` это необязательное переопределение адреса, а не учётные данные. Добавляйте его только если нужно заменить опубликованного совместимого посредника.

Запросите у пользователя фактические значения для заполнения.

#### Шаблон ответа об отсутствующем секрете

При отсутствии значений в навыке с проверкой подлинности получите их согласно порядку разрешения учётных данных.
...
- Проверка пыли по местоположению: обычно ничего, потому что опубликованный совместимый посредник используется по умолчанию; для прямого резервного доступа нужен `AIR_KOREA_OPEN_API_KEY`, а `KSKILL_PROXY_BASE_URL` остаётся только необязательным переопределением.
...
- Даже у устаревшего псевдонима артефакты выполнения должны по умолчанию находиться в `~/.config/ru-skill/*`; путь `~/.config/k-skill/*` допустим только как обратно совместимый резерв, если пользователь уже завязал на него свою локальную автоматизацию
...
#### 4. Предложение отметки звездой на GitHub с явного согласия
...
- При отсутствии `gh` или если `gh` не прошёл проверку подлинности, только проинструктируйте по установке/входу, не используйте обходные пути
- Целевой репозиторий для отметки звездой: `denis-gordeev/ru-skill`
...
- Автоматическая проверка обновлений или отметка звездой на GitHub настроены только по желанию пользователя
...
- Артефакты выполнения по умолчанию находятся в `~/.config/ru-skill/bin` и `~/.config/ru-skill/logs`; устаревший `~/.config/k-skill/*` допустим только как обратно совместимый запасной вариант
