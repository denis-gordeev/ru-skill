...
Этот навык остаётся `устаревший без развития`: он полезен для обратной совместимости и как эталон сценария отслеживания на основе адаптеров, но не считается следующим российским целевым направлением репозитория. Прямая российская замена для отслеживания доставки через публичные бесплатные API пока не подтверждена. Адаптер перевозчика сохранён как обратно совместимый расширяемый паттерн, а не как активный перечень задач для новых курьерских интеграций.
...
| адаптер перевозчика | официальный вход | транспорт | валидатор | фокус парсера |
| --- | --- | --- | --- | --- |
...
- Добавление новых перевозчиков поддерживается через шаблон адаптера, но не является активным направлением развития.
