...
Этот навык остаётся `устаревший без развития`: он полезен для обратной совместимости и как эталон сценария отслеживания на основе модулей-посредников, но не считается следующим российским целевым направлением репозитория. Прямая российская замена для отслеживания доставки через публичные бесплатные API пока не подтверждена. Модуль-посредник перевозчика сохранён как обратно совместимый расширяемый паттерн, а не как активный перечень задач для новых курьерских интеграций.

## Что делает навык

Получает текущий статус доставки по номеру накладной через официальные поверхности CJ Logistics и Почтовой службы Кореи.

- **CJ Logistics**: используется JSON-конечная точка, который раскрывает официальная страница отслеживания доставки
- **Почтовая служба Кореи**: используется HTML-конечная точка, который использует официальная страница отслеживания доставки
...
- `валидатор`: длина/паттерн номера накладной
- `точка входа`: официальный URL для запроса статуса
- `транспорт`: что используется — JSON API / HTML-форма / CLI
- `парсер`: из каких полей или таблиц извлекается статус
- `таблица статусов`: как привести исходные коды статусов перевозчика к общим статусам
- `политика повторных попыток`: правила таймаута и повторных попыток

Текущие модули-посредники:

| модуль-посредник перевозчика | официальный вход | транспорт | модуль проверки | фокус программы разбора |
| --- | --- | --- | --- | --- |
...
- Базовая поверхность использует только официальные конечные точки перевозчиков.
