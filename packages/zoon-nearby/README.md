...
Дополнительный источник для поиска ближайших заведений через публичные страницы Zoon.ru с серверным рендерингом (SSR).
...
- **Серверный рендеринг (SSR)**: HTML генерируется на сервере и напрямую разбирается
