...
`daiso-product-search` - устаревший пакет только для чтения для поиска товаров и остатков для самовывоза в Daiso Mall по официальным веб-интерфейсам.

## Граничное примечание

Этот пакет остаётся `устаревший без развития`: для российского обнаружения на торговой площадке его замена уже реализована как `yandex-market-search`. `daiso-product-search` сохраняется ради обратной совместимости и как эталонный сценарий для потоков магазин/товар/наличие, но не должен выглядеть как активное направление целевой линейки репозитория.
...
- Приоритет всегда у официальных интерфейсов Daiso Mall.
...
На 2026-03-27 комбинация `storeQuery=강남역2호점` и `productQuery=VT 리들샷 100` вернула через официальный интерфейс следующие магазин/товар/наличие данные:
