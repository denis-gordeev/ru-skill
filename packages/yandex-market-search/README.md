...
Возвращает `{query, source, page, results}`, где каждый результат содержит `productId`, `title`, `price`, `rating`, `reviewCount`, `url`, `imageUrl` и основные `specs`.

### `getProduct(productUrl, opts?)`

Получение карточки товара по полному или относительному адресу Яндекс Маркета.
...
Формирование публичного адреса поиска на Яндекс Маркете.

### `buildProductUrl(slug, productId)`

Формирование канонического адреса карточки товара по идентификатору категории и идентификатору товара.

## Примечания

- ключ программного интерфейса и проверка подлинности не требуются.
- Пакет работает только с публичным HTML, сгенерированным на сервере.
- Страницы поиска и карточек содержат много параметров отслеживания; возвращаемые адреса нормализуются до канонических `/card/{идентификатор_категории}/{id}`.
- Только для чтения: нет корзины, оплаты, избранного или действий с учётной записью продавца.
