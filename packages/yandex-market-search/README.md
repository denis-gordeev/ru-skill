...
Клиент только для чтения для поиска товаров и карточек товаров на серверно отрендеренных страницах [Яндекс Маркета](https://market.yandex.ru/).
...
- Страницы поиска и карточек содержат много трекинг-параметров; возвращаемые URL нормализуются до канонических `/card/{идентификатор_категории}/{id}`.
