...
`k-lotto` - legacy-пакет для результатов корейской лотереи 6/45 через официальные страницы и JSON-ответы `동행복권`.

## Boundary note

Этот пакет остаётся `legacy-only`: для российских публичных сценариев его replacement уже реализован как `stoloto-lotto`. `k-lotto` сохраняется только ради backward compatibility и не должен продвигаться как новый пользовательский сценарий `ru-skill`.
