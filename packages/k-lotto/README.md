...
`k-lotto` - устаревший пакет для результатов корейской лотереи 6/45 через официальные страницы и ответы в формате JSON от Dhlottery.

## Граничное примечание

Этот пакет остаётся `устаревший без развития`: для российских публичных сценариев его замена уже реализована как `stoloto-lotto`. `k-lotto` сохраняется только ради обратной совместимости и не должен продвигаться как новый пользовательский сценарий `ru-skill`.
