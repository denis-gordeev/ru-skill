...
Этот навык остаётся как `legacy-only`. Замена для российских публичных результатов лотерей уже реализована в `stoloto-lotto`, поэтому `lotto-results` не рассматривается как новый целевой кандидат.
