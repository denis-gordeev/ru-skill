...
Этот навык предполагает **macOS + установленное приложение KakaoTalk для Mac**. Он работает не через официальный программный интерфейс Kakao, а поверх чтения локальной базы данных и автоматизации специальных возможностей macOS, поэтому сначала нужно проверить разрешения и правила безопасности.

## Граничное примечание

Этот навык остаётся на `устаревший без развития` границе. Он поддерживается как локальный эталон автоматизации macOS и для обратной совместимости, но не рассматривается как новое русскоязычное целевое направление обмена сообщениями или скрытый перечень задач.
...
- Задача межсерверной интеграции, которую можно решить через официальный программный интерфейс Kakao
...
- `kakaocli` установлен
- Предоставлены разрешения **Полный доступ к диску (Full Disk Access)** и **Универсальный доступ (Accessibility)** для приложения терминала
...
- Признак тестовой отправки (`--me`, `--dry-run`)
...
В **Системные настройки > Конфиденциальность и защита (System Settings > Privacy & Security)** предоставьте текущему приложению терминала (iTerm, Terminal, Warp и т.д.) следующие разрешения:

- **Полный доступ к диску (Full Disk Access)**: для чтения локальной базы данных KakaoTalk
- **Универсальный доступ (Accessibility)**: для автоматизации интерфейса — отправка сообщений, сбор данных, проверка

Основные правила:

- Команды чтения (`status`, `auth`, `chats`) также требуют Полный доступ к диску (Full Disk Access).
- Команды `send`, `harvest`, `inspect` дополнительно требуют Универсальный доступ (Accessibility).

### 3. Проверить доступ на чтение перед действиями с побочными эффектами
...
Получать ответы предпочтительно в режиме JSON и кратко пересказывать для удобства чтения.

### 5. Использовать безопасное тестирование перед реальной отправкой

Перед фактической отправкой сначала протестировать на себе или проверить через пробный запуск.
...
- Полный доступ к диску (Full Disk Access) не предоставлен
- Универсальный доступ (Accessibility) не предоставлен
