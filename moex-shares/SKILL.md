---
name: moex-shares
description: Проверка публичных метаданных акций Московской биржи и задержанных срезов рынка через официальный программный интерфейс ISS. Использовать, когда пользователь спрашивает данные по тикеру MOEX: SBER, GAZP, LKOH, размер лота, ISIN, предыдущее закрытие или текущий задержанный срез цены.
license: MIT
metadata:
  category: финансы
  locale: ru-RU
...
Пакет `moex-shares` получает по публичному программному интерфейсу ISS Московской биржи базовые метаданные акции и отложенный срез рынка по тикеру.
