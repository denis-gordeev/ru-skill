...
Этот навык остаётся `устаревший без развития`: прямая российская замена для брокерских сценариев с проверкой подлинности не подтверждена, а публичные рыночные сводки только для чтения уже покрываются `moex-shares`. `toss-securities` сохраняется ради обратной совместимости с существующими корейскими брокерскими сценариями, но не считается новым целевым направлением `ru-skill`.
...
- macOS + Homebrew
- `tossctl` установлен
- Сеанс обозревателя получен через `tossctl auth login`
- Node.js 18+
...
Если сеанс входа отсутствует, сначала завершить этот поток. Не обходить через другой неофициальный сбор данных или произвольную реализацию HTTP.

### 1. Предпочитать интерфейс `tossctl` только для чтения
...
- При изменении структуры вышестоящего программного интерфейса может потребоваться обновление самого `tossctl`
