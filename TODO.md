...
## Статус на 2026-07-20 (раунд 100)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит интерфейсной документации выявил систематическое использование `авторизация` (~35 вхождений в ~20 файлах) как синонима `аутентификация`, несмотря на уже устранённый жаргонизм `аутентификация` → `проверка подлинности`. Все вхождения `авторизация` заменены на `проверка подлинности` для согласованности.
- 10 ключевых жаргонизмов (`релиз`, `прокси`, `кэш`, `эндпоинт`, `парсинг`, `токен`, `аутентификация`, `хост`, `аккаунт`, `логин`) в интерфейсной документации не обнаружены; `авторизация` добавлена к контролируемому перечню.

## Выполнено в этом раунде (раунд 100)

- [x] `mchs-storm-warnings/SKILL.md`: `авторизации` → `проверки подлинности`.
- [x] `packages/osm-nearby/SKILL.md`: `авторизации` → `проверки подлинности` (2 вхождения).
- [x] `pravo-documents/SKILL.md`: `авторизации` → `проверки подлинности`.
- [x] `hh-vacancies/SKILL.md`: `авторизации` → `проверки подлинности`.
- [x] `packages/zoon-nearby/README.md`: `авторизации` → `проверки подлинности`.
- [x] `packages/kinopoisk-search/README.md`: `авторизации` → `проверки подлинности`.
- [x] `packages/yandex-market-search/README.md`: `авторизация не требуются` → `проверка подлинности не требуются`.
- [x] `packages/stoloto-lotto/README.md`: `авторизации` → `проверки подлинности`.
- [x] `packages/pravo-documents/README.md`: `авторизации` → `проверки подлинности`.
- [x] `packages/rpl-results/README.md`: `авторизации` → `проверки подлинности` (2 вхождения).
- [x] `packages/mchs-storm-warnings/README.md`: `авторизации` → `проверки подлинности`.
- [x] `docs/features/k-skill-proxy.md`: `авторизации посредника` → `проверки подлинности посредника`, `авторизацией` → `проверкой подлинности`.
- [x] `docs/features/rpl-results.md`: `авторизации` → `проверки подлинности` (2 вхождения).
- [x] `docs/features/stoloto-lotto.md`: `авторизации` → `проверки подлинности`.
- [x] `docs/features/kinopoisk-search.md`: `авторизации` → `проверки подлинности`.
- [x] `docs/install.md`: `авторизации` → `проверки подлинности` (2 вхождения).
- [x] `docs/sources.md`: `авторизация`/`авторизации` → `проверка подлинности`/`проверки подлинности` (9 вхождений).
- [x] `docs/setup.md`: `авторизацией` → `проверкой подлинности`.
- [x] `docs/security-and-secrets.md`: `авторизацией` → `проверкой подлинности`.
- [x] `docs/roadmap.md`: `авторизации` → `проверки подлинности`.
- [x] `AGENTS.md`: `авторизации посредника` → `проверки подлинности посредника`, `авторизацию` → `проверку подлинности`.
- [x] `scripts/skill-docs.test.js`: `авторизацию` → `проверку подлинности` в описании теста, `/авторизации посредника/` → `/проверки подлинности посредника/` в утверждении.
- [x] Периодический аудит интерфейсной документации: 10 ключевых жаргонизмов не вернулись; `авторизация` устранена.
