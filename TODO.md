...
- Скрипт `scripts/fix-changelog-headings.js` усилен обработкой ошибок и русскими статусными сообщениями; добавлен регрессионный тест на безопасную повторную обработку (идемпотентность).
- Обработчик ошибки запуска `packages/k-skill-proxy/src/server.js` русифицирован: `console.error(error)` → `console.error("Не удалось запустить сервер-посредник:", error.message)`.
- Периодический аудит выявил 2 возврата `HH API` в `docs/sources.md` и `hh-vacancies/SKILL.md`; заменено на `программному интерфейсу HH` / `программного интерфейса HH`.
- Остальные 11 ключевых жаргонизмов не вернулись в интерфейсной документации; оставшиеся `API` — в собственных именах (ISS API, Overpass API и др.).

## Выполнено в этом раунде (раунд 105)

- [x] `scripts/fix-changelog-headings.js`: добавлена обработка ошибок чтения/записи, русские статусные сообщения, проверка существования каталога пакетов.
- [x] `scripts/skill-docs.test.js`: добавлен регрессионный тест на идемпотентность `fix-changelog-headings` (повторная обработка не изменяет русские заголовки).
- [x] `scripts/skill-docs.test.js`: добавлен регрессионный тест на русские статусные сообщения `fix-changelog-headings`.
- [x] `packages/k-skill-proxy/src/server.js`: обработчик ошибки запуска русифицирован.
- [x] `scripts/skill-docs.test.js`: добавлен регрессионный тест на русские сообщения `k-skill-proxy` при ошибке запуска.
- [x] `docs/sources.md`: `HH API` → `программного интерфейса HH` (1 вхождение).
- [x] `hh-vacancies/SKILL.md`: `HH API` → `программному интерфейсу HH` (1 вхождение).
- [x] Периодический аудит интерфейсной документации: 11 ключевых жаргонизмов не вернулись; `API` как самостоятельное существительное устранено (оставшиеся `API` — в собственных именах).
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана

- [ ] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона.
