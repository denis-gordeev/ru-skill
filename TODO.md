...
## Статус на 2026-08-26 (раунд 135)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит выявил и устранил: `# HH вакансии` — английский-первый h1-заголовок в `docs/features/hh-vacancies.md` (1 вхождение: → `# Вакансии HH`), `# Postcalc и индексы Почты России` — английский-первый h1-заголовок в `docs/features/postcalc-postcodes.md` (1 вхождение: → `# Почтовые индексы Postcalc`), `{slug}` — непояснённый английский шаблонный параметр адреса в `docs/features/yandex-market-search.md` (1 вхождение: → `{идентификатор_категории}`, приведено к формату `packages/yandex-market-search/README.md`).
- Периодический аудит не выявил возвратов 23 ключевых жаргонизмов в интерфейсной документации.

## Выполнено в этом раунде (раунд 135)

- [x] `docs/features/hh-vacancies.md`: `# HH вакансии` → `# Вакансии HH` (1 вхождение, английский-первый h1-заголовок приведён к русский-первый порядок, согласовано с SKILL.md).
- [x] `docs/features/postcalc-postcodes.md`: `# Postcalc и индексы Почты России` → `# Почтовые индексы Postcalc` (1 вхождение, английский-первый h1-заголовок приведён к русский-первый порядок, согласовано с SKILL.md).
- [x] `docs/features/yandex-market-search.md`: `{slug}` → `{идентификатор_категории}` (1 вхождение, непояснённый шаблонный параметр адреса заменён на русский, согласовано с `packages/yandex-market-search/README.md`).
- [x] `scripts/skill-docs.test.js`: добавлены регрессионные проверки на русский-первый порядок h1-заголовков в `docs/features/hh-vacancies.md` и `docs/features/postcalc-postcodes.md`, а также на отсутствие `{slug}` и наличие `{идентификатор_категории}` в `docs/features/yandex-market-search.md`.
- [x] Периодический аудит интерфейсной документации: 23 ключевых жаргонизма не вернулись; английские-первые h1-заголовки и `{slug}` устранены.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда 135.
...
## Статус на 2026-08-25 (раунд 134)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Завершён давний пункт плана: английские идентификаторы в возвращаемых значениях устаревших пакетов (`kleague-results`: `state`, `winner`; `blue-ribbon-nearby`: `matchedBy`) теперь пояснены на русском в документации README.
- Периодический аудит выявил и устранил: `JSON-файл` — английский-первый гибрид в справке интерфейса командной строки `scripts/fine_dust.py` (2 вхождения: → `файл JSON`).
- Периодический аудит не выявил возвратов 23 ключевых жаргонизмов в интерфейсной документации.

## Выполнено в этом раунде (раунд 134)

- [x] `packages/kleague-results/README.md`: добавлена секция «Возвращаемые значения» с пояснением `state (состояние)`, `winner (победитель)` и их допустимых значений на русском.
- [x] `packages/blue-ribbon-nearby/README.md`: добавлена секция «Возвращаемые значения» с пояснением `matchedBy (способ сопоставления)` и его допустимых значений на русском.
- [x] `scripts/fine_dust.py`: `JSON-файл` → `файл JSON` (2 вхождения: строки 53 и 54, приведено к русский-первый порядок).
- [x] `scripts/skill-docs.test.js`: добавлены регрессионные проверки на `state (состояние)`, `winner (победитель)`, `matchedBy (способ сопоставления)` в README пакетов и на отсутствие `JSON-файл` в `scripts/fine_dust.py`.
- [x] Периодический аудит интерфейсной документации: 23 ключевых жаргонизма не вернулись; `JSON-файл` устранён.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда 134.

## Новые пункты плана

- [x] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона, включая глубокие секции `docs/roadmap.md` и `TODO.md` (выполнено в раунде 135).

## Статус на 2026-08-24 (раунд 133)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Завершён оставшийся после раунда 132 слой пояснения английских идентификаторов в пользовательских ошибках: идентификаторы полей ответов и элементов XML, которые до этого выводились без русского пояснения, теперь используют единый формат `identifier (русское пояснение)`.
- Периодический аудит не выявил возвратов 23 ключевых жаргонизмов в интерфейсной документации.

## Выполнено в этом раунде (раунд 133)

- [x] `hh-vacancies/src/index.js`: `"areaId"` → `"areaId (идентификатор региона)"` (2 вхождения: строки 68 и 108, приведены к формату строки 47).
- [x] `hh-vacancies/src/index.js`: `"vacancyId"` → `"vacancyId (идентификатор вакансии)"` (1 вхождение: строка 129, приведено к формату строки 55).
- [x] `hh-vacancies/src/parse.js`: `массив items` → `массив items (список вакансий)` (1 вхождение).
- [x] `moex-shares/src/parse.js`: `массивами columns и data` → `массивами columns (столбцы) и data (данные)` (1 вхождение).
- [x] `moex-shares/src/parse.js`: `строку securities` → `строку securities (ценные бумаги)` (1 вхождение).
- [x] `moex-shares/src/parse.js`: `строку marketdata` → `строку marketdata (рыночные данные)` (1 вхождение).
- [x] `cbr-rates/src/parse.js`: `<ValCurs>` → `<ValCurs> (курсы валют)` (1 вхождение).
- [x] `cbr-rates/src/parse.js`: `<Valute>` → `<Valute> (валюты)` (1 вхождение).
- [x] `cbr-rates/test/index.test.js`: `date_req` → `date_req (запрошенная дата)` (1 вхождение в сообщении имитированной ошибки).
- [x] `yandex-rasp/src/index.js`: `opts.apiKey или` → `opts.apiKey (ключ) или` (1 вхождение).
- [x] `toss-securities/src/parse.js`: `all, us, kr` → `all (все), us (США), kr (Корея)` (1 вхождение, пояснены допустимые значения перечисления без изменения публичного контракта).
- [x] `scripts/skill-docs.test.js`: добавлены регрессионные проверки на `items (список вакансий)`, `columns (столбцы)`, `data (данные)`, `securities (ценные бумаги)`, `marketdata (рыночные данные)`, `ValCurs> (курсы валют)`, `Valute> (валюты)`, `opts.apiKey (ключ)`, `all (все)`, `us (США)`, `kr (Корея)`.
- [x] Периодический аудит интерфейсной документации: 23 ключевых жаргонизма не вернулись.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда 133.

## Новые пункты плана

- [x] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона, включая глубокие секции `docs/roadmap.md` и `TODO.md` (выполнено в раунде 134).
- [x] Проверить оставшиеся английские идентификаторы в возвращаемых значениях устаревших пакетов (`kleague-results`: `state`, `winner`; `blue-ribbon-nearby`: `matchedBy`), где добавление поля `label` рядом с программным значением не меняет публичный контракт (выполнено в раунде 134: `state` уже имеет `status.label`, добавлены `winnerLabel` и `matchedByLabel`).

## Статус на 2026-08-24 (раунд 132)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Завершён крупный слой пояснения английских идентификаторов в пользовательских ошибках: 9 целевых и 6 устаревших пакетов, а также `k-skill-proxy` теперь используют единый формат `parameterName (русское пояснение)`.
- Документная регрессия расширена двумя новыми тестами: один проверяет пояснение имён параметров во всех целевых и устаревших пакетах, другой — пояснение идентификаторов в `k-skill-proxy`.

## Выполнено в этом раунде (раунд 132)

- [x] `hh-vacancies`: `page` → `page (номер страницы)`, `perPage` → `perPage (элементов на страницу)`, `areaId` → `areaId (идентификатор региона)`, `vacancyId` → `vacancyId (идентификатор вакансии)` (4 вхождения).
- [x] `pravo-documents`: `page` → `page (номер страницы)`, `pageSize` → `pageSize (размер страницы)`, `eoNumber` → `eoNumber (номер электронной публикации)` (3 вхождения).
- [x] `mchs-storm-warnings`: `page` → `page (номер страницы)`, `warningPathOrId` → `warningPathOrId (путь или идентификатор предупреждения)` (3 вхождения), `regionHost` → `regionHost (имя узла региона)` (1 вхождение).
- [x] `moex-shares`: `secId` → `secId (идентификатор ценной бумаги)` (1 вхождение).
- [x] `cbr-rates`: `date` → `date (дата)`, `charCode` → `charCode (буквенный код валюты)` (2 вхождения).
- [x] `yandex-market-search`: `productUrl` → `productUrl (адрес товара)` (1 вхождение).
- [x] `postcalc-postcodes`: `postalCode` → `postalCode (почтовый индекс)`, `cityKey` → `cityKey (ключ города)` (2 вхождения).
- [x] `blue-ribbon-nearby`: `distanceMeters` → `distanceMeters (расстояние в метрах)`, `latitude`/`longitude` → `latitude (широта)`/`longitude (долгота)`, `lat/lng` → `широта/долгота` (4 вхождения).
- [x] `daiso-product-search`: `storeQuery` → `storeQuery (поисковый запрос магазина)`, `productQuery` → `productQuery (поисковый запрос товара)` (2 вхождения).
- [x] `k-lotto`: `round` → `round (номер тиража)`, `ticketNumbers` → `ticketNumbers (номера билета)` (4 вхождения).
- [x] `kakao-bar-nearby`: `locationQuery` → `locationQuery (поисковый запрос местоположения)` (1 вхождение).
- [x] `kleague-results`: `leagueId` → `leagueId (идентификатор лиги)`, `date` → `date (дата)` (4 вхождения).
- [x] `toss-securities`: `tossctl` → `tossctl (утилита командной строки)`, `market` → `market (рынок)`, `symbol` → `symbol (тикер)`, `symbols` → `symbols (список тикеров)` (7 вхождений).
- [x] `k-skill-proxy/airkorea.js`: `403 Forbidden` → `403 (доступ запрещён)`, `AIR_KOREA_OPEN_API_KEY` → `AIR_KOREA_OPEN_API_KEY (ключ доступа к программному интерфейсу)`, `regionHint` → `regionHint (подсказка региона)`, `stationName` → `stationName (название станции)` (7 вхождений); запасной вариант поиска измерений обновлён под новый текст ошибки.
- [x] `k-skill-proxy/server.js`: `regionHint` → `regionHint (подсказка региона)`, `stationName` → `stationName (название станции)`, `AIR_KOREA_OPEN_API_KEY` → `AIR_KOREA_OPEN_API_KEY (ключ доступа к программному интерфейсу)` (5 вхождений).
- [x] `scripts/skill-docs.test.js`: добавлены регрессионные тесты «пользовательские ошибки целевых и устаревших пакетов поясняют английские имена параметров на русском» и «пользовательские сообщения k-skill-proxy поясняют английские идентификаторы на русском»; обновлены 4 существующих теста под новый формат.
- [x] `packages/toss-securities/test/index.test.js`: обновлён паттерн `assert.throws` под новый текст ошибки.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда 132.

## Новые пункты плана

- [x] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона, включая глубокие секции `docs/roadmap.md` и `TODO.md` (выполнено в раунде 133).

## Статус на 2026-08-18 (раунд 130)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит выявил: `CLI` в именах продуктов без русского пояснения в 6 файлах (6 вхождений: `1Password CLI`, `Bitwarden CLI` → добавлено пояснение `(интерфейсы командной строки)` в `docs/setup.md`, `docs/security-and-secrets.md`, `k-skill-setup/SKILL.md`, `srt-booking/SKILL.md`, `ktx-booking/SKILL.md`, `seoul-subway-arrival/SKILL.md`), английские заголовки и метки в `tg/*/README.md` (4 файла: `# My-RU-Coverage` → `# My-RU-Coverage (зеркальный канал)`, `- Repo:` → `- Репозиторий:`, `pending` → `ожидается`), английский h1-заголовок в `packages/k-skill-proxy/README.md` (`# k-skill-proxy` → `# Сервер-посредник k-skill-proxy`), `Fastify` без обратных кавычек в `packages/k-skill-proxy/README.md` (1 вхождение: `на Fastify` → `на `Fastify``), `station-list` без обратных кавычек в `docs/features/fine-dust-location.md` (1 вхождение); все устранены.
- Все 23 ключевых жаргонизма (`API`, `CLI`, `MVP`, `legacy`, `парсинг`, `авторизация`, `логин`, `прокси`, `URL`, `ID`, `ботов`, `пагинация`, `эндпоинт`, `аутентификация`, `кэш`, `хост`, `аккаунт`, `рендеринг`, `поверхность`, `валидация`, `моки`, `лог-сообщения`, `релиз`, `браузерный`) не вернулись в пользовательских документах.

## Выполнено в этом раунде (раунд 130)

- [x] 6 файлов: `1Password CLI`, `Bitwarden CLI` дополнены пояснением `(интерфейсы командной строки)` в `docs/setup.md`, `docs/security-and-secrets.md`, `k-skill-setup/SKILL.md`, `srt-booking/SKILL.md`, `ktx-booking/SKILL.md`, `seoul-subway-arrival/SKILL.md` (6 вхождений).
- [x] 4 файла `tg/*/README.md`: английский h1-заголовок дополнен русским подзаголовком `(зеркальный канал)`, `- Repo:` → `- Репозиторий:`, `pending` → `ожидается` (4 файла × 3 изменения).
- [x] `packages/k-skill-proxy/README.md`: `# k-skill-proxy` → `# Сервер-посредник k-skill-proxy` (1 вхождение, английский h1-заголовок приведён к русскому).
- [x] `packages/k-skill-proxy/README.md`: `на Fastify` → `` на `Fastify` `` (1 вхождение, имя технологии обёрнуто в обратные кавычки).
- [x] `docs/features/fine-dust-location.md`: `программный интерфейс station-list` → `` программный интерфейс `station-list` `` (1 вхождение, идентификатор обёрнут в обратные кавычки).
- [x] `scripts/skill-docs.test.js`: ожидаемый паттерн в тесте для `packages/k-skill-proxy/README.md` обновлён под `Fastify` в обратных кавычках.
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; `CLI` без русского пояснения, английские заголовки и метки в tg/, английский h1 и `Fastify` без кавычек устранены.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана (перенесены из раунда 129 — выполнены в раунде 130)
...
## Статус на 2026-08-17 (раунд 129)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит выявил возврат жаргонизмов и гибридов в `scripts/skill-docs.test.js`: `поверхность` → `интерфейс` (24 вхождения), `legacy` в связном тексте → `устаревший` (~21 вхождение в именах тестов и сообщениях утверждений), английские-первые гибриды → русские-первые (~28 вхождений: `npm-скрипт` → `скрипт npm`, `Python-хелпера` → `вспомогательного скрипта Python`, `workspace-пакеты` → `пакеты рабочего пространства`, `shell-скрипты` → `скрипты оболочки`, `target-пакетов` → `целевых пакетов` и др.); `CI` без русского пояснения → `системе непрерывной интеграции (CI)` в `docs/roadmap.md` (1 вхождение).
- Все 23 ключевых жаргонизма (`API`, `CLI`, `MVP`, `legacy`, `парсинг`, `авторизация`, `логин`, `прокси`, `URL`, `ID`, `ботов`, `пагинация`, `эндпоинт`, `аутентификация`, `кэш`, `хост`, `аккаунт`, `рендеринг`, `поверхность`, `валидация`, `моки`, `лог-сообщения`, `релиз`, `браузерный`) не вернулись в пользовательских документах.

## Выполнено в этом раунде (раунд 129)

- [x] `scripts/skill-docs.test.js`: `поверхность` → `интерфейс` (24 вхождения в именах тестов и сообщениях утверждений).
- [x] `scripts/skill-docs.test.js`: `legacy` → `устаревший`/`устаревших`/`устаревшая` в именах тестов и сообщениях утверждений (~21 вхождение).
- [x] `scripts/skill-docs.test.js`: английские-первые гибриды заменены на русские-первые (~28 вхождений: `npm-скрипт` → `скрипт npm`, `Python-хелпера` → `вспомогательного скрипта Python`, `workspace-пакеты` → `пакеты рабочего пространства`, `shell-скрипты` → `скрипты оболочки`, `target-пакетов` → `целевых пакетов`, `inline-характеристики` → `встроенные характеристики`, `anchor-точка` → `опорная точка`, `booking-навыков` → `навыков бронирования`, `Legacy-формулировка` → `устаревшая формулировка`, `provenance-метаданные` → `метаданные происхождения`, `amenity-типы` → `типы заведений`, `secrets-файл` → `файл секретов`, `zone-списка` → `списка зон`, `accessibility-автоматизации` → `автоматизации специальных возможностей` и др.).
- [x] `docs/roadmap.md`: `в CI` → `в системе непрерывной интеграции (CI)` (1 вхождение).
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись в пользовательских документах; `поверхность`, `legacy` и английские-первые гибриды в тестовых описаниях устранены.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана

- [x] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона, включая глубокие секции `docs/roadmap.md` и `TODO.md` (выполнено в раунде 130).

## Статус на 2026-08-14 (раунд 128)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит не выявил возвратов ключевых жаргонизмов в интерфейсной документации; устранено пороговое несоответствие: `серверный рендеринг (SSR)` → `серверная отрисовка (SSR)` в исторической заметке `README.md` (1 вхождение).
- Все 23 ключевых жаргонизма (`API`, `CLI`, `MVP`, `legacy`, `парсинг`, `авторизация`, `логин`, `прокси`, `URL`, `ID`, `ботов`, `пагинация`, `эндпоинт`, `аутентификация`, `кэш`, `хост`, `аккаунт`, `рендеринг`, `поверхность`, `валидация`, `моки`, `лог-сообщения`, `релиз`, `браузерный`) не вернулись в пользовательских документах.

## Выполнено в этом раунде (раунд 128)

- [x] `README.md`: `серверный рендеринг (SSR)` → `серверная отрисовка (SSR)` (1 вхождение в исторической заметке, приведено к каноничной форме).
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; пороговое несоответствие формулировки SSR устранено.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана (перенесены из раунда 127 — выполнены в раунде 128)

- [x] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона, включая глубокие секции `docs/roadmap.md` и `TODO.md`.

## Статус на 2026-08-14 (раунд 127)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит выявил: `XML-курсов` — английский-первый гибрид в `packages/cbr-rates/package.json` (1 вхождение: → `курсов валют в формате XML`), `OIDC (OpenID Connect — открытая идентификация)` — английская аббревиатура впереди русского пояснения в `AGENTS.md` и `docs/releasing.md` (3 вхождения: → `протокол открытой идентификации (OIDC)` / `открытая идентификация (OIDC)`), `push-уведомления` — английский-первый гибрид в `docs/sources.md` (2 вхождения: → `уведомления push`); все устранены.
- Все 23 ключевых жаргонизма (`API`, `CLI`, `MVP`, `legacy`, `парсинг`, `авторизация`, `логин`, `прокси`, `URL`, `ID`, `ботов`, `пагинация`, `эндпоинт`, `аутентификация`, `кэш`, `хост`, `аккаунт`, `рендеринг`, `поверхность`, `валидация`, `моки`, `лог-сообщения`, `релиз`, `браузерный`) не вернулись в пользовательских документах.

## Выполнено в этом раунде (раунд 127)

- [x] `packages/cbr-rates/package.json`: `XML-курсов` → `курсов валют в формате XML` (1 вхождение, приведено к русский-первый порядок).
- [x] `AGENTS.md`: `OIDC (OpenID Connect — открытая идентификация)` → `протокол открытой идентификации (OIDC)` (1 вхождение, приведено к русский-первый порядок).
- [x] `docs/releasing.md`: `GitHub OIDC (OpenID Connect — открытая идентификация)` → `протокол открытой идентификации GitHub (OIDC)` (1 вхождение), `OIDC (OpenID Connect — открытая идентификация)` → `открытая идентификация (OIDC)` (1 вхождение).
- [x] `docs/sources.md`: `push-уведомления` → `уведомления push` (2 вхождения, приведено к русский-первый порядок).
- [x] `scripts/skill-docs.test.js`: ожидаемый description для `cbr-rates` обновлён под `курсов валют в формате XML`.
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; `XML-курсов`, `OIDC` впереди русского пояснения и `push-уведомления` устранены.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана (перенесены из раунда 127 — выполнены в раундах 128–129)

- [x] Продолжать периодический аудит интерфейсной документации на новые гибриды и возврат английского жаргона, включая глубокие секции `docs/roadmap.md` и `TODO.md`.

## Статус на 2026-08-13 (раунд 126)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит выявил: `CI` без русского пояснения в `README.md` (1 вхождение: `в CI` → `в системе непрерывной интеграции (CI)`), `OpenAPI` без русского пояснения в `docs/features/fine-dust-location.md` (1 вхождение: `ключ Air Korea OpenAPI` → `ключ открытого программного интерфейса Air Korea`); все устранены.
- Все 23 ключевых жаргонизма (`API`, `CLI`, `MVP`, `legacy`, `парсинг`, `авторизация`, `логин`, `прокси`, `URL`, `ID`, `ботов`, `пагинация`, `эндпоинт`, `аутентификация`, `кэш`, `хост`, `аккаунт`, `рендеринг`, `поверхность`, `валидация`, `моки`, `лог-сообщения`, `релиз`, `браузерный`) не вернулись в пользовательских документах.

## Выполнено в этом раунде (раунд 126)

- [x] `README.md`: `в CI` → `в системе непрерывной интеграции (CI)` (1 вхождение).
- [x] `docs/features/fine-dust-location.md`: `ключ Air Korea OpenAPI` → `ключ открытого программного интерфейса Air Korea` (1 вхождение).
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; `CI` и `OpenAPI` без русского пояснения устранены.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана (перенесены из раунда 126 — выполнены в раунде 127)
...
- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит выявил: `URL` как самостоятельное существительное в JSDoc и описаниях тестов (7 пакетов: `kinopoisk-search`, `rpl-results`, `pravo-documents`, `mchs-storm-warnings`, `zoon-nearby` ×2, `osm-nearby` — 12 вхождений в JSDoc), `URL` в описаниях тестов (9 пакетов: `hh-vacancies`, `moex-shares`, `postcalc-postcodes`, `mchs-storm-warnings` ×2, `rpl-results` ×3, `kinopoisk-search` ×2, `pravo-documents` ×2, `yandex-market-search`, `zoon-nearby` ×3 — 16 вхождений), `неожиданный URL` в сообщениях об ошибках (3 пакета: `kakao-bar-nearby` ×4, `kleague-results` ×1, `k-skill-proxy/test/airkorea.test.js` ×3 — 8 вхождений), `unexpected URL` в `scripts/test_fine_dust.py` (3 вхождения); все заменены на `адрес`/`адресов`/`неожиданный адрес`.
- Остальные ключевые жаргонизмы не вернулись в интерфейсной документации.

## Выполнено в этом раунде (раунд 124)

- [x] `packages/kinopoisk-search/src/index.js`: `Построить URL страницы фильма` → `Построить адрес страницы фильма`, `Построить URL страницы поиска` → `Построить адрес страницы поиска` (2 вхождения в JSDoc).
- [x] `packages/rpl-results/src/index.js`: `Построить URL страницы турнирной таблицы` → `Построить адрес страницы турнирной таблицы`, `Построить URL страницы результатов матчей` → `Построить адрес страницы результатов матчей` (2 вхождения в JSDoc).
- [x] `packages/pravo-documents/src/parse.js`: `Построить URL поиска` → `Построить адрес поиска`, `Построить URL карточки документа` → `Построить адрес карточки документа` (2 вхождения в JSDoc).
- [x] `packages/mchs-storm-warnings/src/index.js`: `абсолютным URL МЧС` → `абсолютным адресом МЧС` (1 вхождение в сообщении об ошибке).
- [x] `packages/zoon-nearby/src/parse.js`: `Извлечь URL` → `Извлечь адрес`, `Нормализовать URL` → `Нормализовать адрес`, `Исходный URL` → `Исходный адрес`, `URL страницы для контекста` → `Адрес страницы для контекста` (4 вхождения).
- [x] `packages/zoon-nearby/src/index.js`: `Полный URL страницы` → `Полный адрес страницы` (1 вхождение в JSDoc).
- [x] `packages/osm-nearby/test/index.test.js`: `закодировано в URL` → `закодировано в адресе` (1 вхождение).
- [x] 9 пакетов: `URL` → `адрес`/`адресов`/`адреса` в описаниях тестов (16 вхождений: `построители URL` → `построители адресов` ×3, `фиксирует URL` → `фиксирует адрес` ×2, `возвращает URL` → `возвращает адрес` ×2, `создаёт корректный URL` → `создаёт корректный адрес` ×2, `возвращают URL` → `возвращают адреса` ×1, `абсолютный URL` → `абсолютный адрес` ×1, `относительные URL` → `относительные адреса` ×1, `абсолютные URL` → `абсолютные адреса` ×1, `некорректных URL` → `некорректных адресов` ×1, `Тесты построения URL` → `Тесты построения адресов` ×1, `абсолютным URL` → `абсолютным адресом` ×1).
- [x] 3 пакета: `неожиданный URL` → `неожиданный адрес` в сообщениях об ошибках (8 вхождений: `kakao-bar-nearby` ×4, `kleague-results` ×1, `k-skill-proxy/test/airkorea.test.js` ×3).
- [x] `scripts/test_fine_dust.py`: `unexpected URL` → `неожиданный адрес` (3 вхождения).
- [x] `scripts/skill-docs.test.js`: обновлены регрессионные тесты под `адрес` вместо `URL` в JSDoc и `неожиданный адрес` вместо `неожиданный URL`; добавлен новый регрессионный тест «JSDoc и описания тестов используют „адрес" вместо URL как самостоятельное существительное».
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; `URL` как самостоятельное существительное в JSDoc, тестах и сообщениях об ошибках устранён.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана (перенесены из раунда 123 — выполнены в раунде 124)
...
## Новые пункты плана (перенесены из раунда 120 — выполнены в раундах 121–123)
