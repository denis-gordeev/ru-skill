...
- [ ] Проверить оставшиеся английские идентификаторы в возвращаемых значениях устаревших пакетов (`kleague-results`: `state`, `winner`; `blue-ribbon-nearby`: `matchedBy`), где добавление поля `label` рядом с программным значением не меняет публичный контракт.
