...
- Устранены 5 английских-первых гибридов в сообщениях об ошибках исходного кода: `JSON-объект` → `объект JSON`, `JSON-вывод` → `вывод JSON`, `XML-ответ` → `ответ XML` (3 вхождения).
- Добавлены поля `label` рядом с английскими значениями в возвращаемых объектах устаревших пакетов: `winnerLabel` в `kleague-results` (`"draw"` → `"ничья"`, `"home"` → `"победа хозяев"`, `"away"` → `"победа гостей"`), `matchedByLabel` в `blue-ribbon-nearby` (`"query"` → `"по запросу"`, `"alias"` → `"по псевдониму"`). Публичные контракты не изменены — новые поля чисто добавочные.

## Выполнено в этом раунде (раунд 134)

- [x] `packages/k-lotto/src/parse.js`: `JSON-объект` → `объект JSON` (1 вхождение в сообщении об ошибке).
- [x] `packages/toss-securities/src/parse.js`: `JSON-вывод` → `вывод JSON` (1 вхождение в сообщении об ошибке).
- [x] `packages/cbr-rates/src/parse.js`: `XML-ответ` → `ответ XML` (3 вхождения в сообщениях об ошибках).
- [x] `packages/kleague-results/src/parse.js`: добавлена карта `WINNER_LABEL_MAP` и поле `winnerLabel` рядом с `winner` в возвращаемом объекте матча (`null` → `null`, `"draw"` → `"ничья"`, `"home"` → `"победа хозяев"`, `"away"` → `"победа гостей"`). Публичный контракт не изменён.
- [x] `packages/kleague-results/test/index.test.js`: добавлены проверки `winner` и `winnerLabel` для завершённого матча.
- [x] `packages/blue-ribbon-nearby/src/parse.js`: добавлена карта `MATCHED_BY_LABEL_MAP` и поле `matchedByLabel` рядом с `matchedBy` в возвращаемом объекте кандидата (`"query"` → `"по запросу"`, `"alias"` → `"по псевдониму"`). Публичный контракт не изменён.
- [x] `packages/blue-ribbon-nearby/test/index.test.js`: добавлены проверки `matchedBy`/`matchedByLabel` для обоих сценариев совпадения.
- [x] `scripts/skill-docs.test.js`: обновлён паттерн `вывод JSON` вместо `JSON-вывод`; добавлены регрессионные тесты «устаревшие возвращаемые объекты поясняют английские идентификаторы полей на русском через поля label» и «пользовательские ошибки не содержат английских-первых гибридов в описаниях форматов».
...
- [ ] Проверить оставшиеся английские идентификаторы в возвращаемых значениях устаревших пакетов, где добавление поля `label` рядом с программным значением не меняет публичный контракт (например, `kbo-results`, `kakao-bar-nearby`, `k-lotto`, `daiso-product-search`).
