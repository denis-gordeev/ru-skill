...
- [ ] Выбрать восьмой российский read-only источник, чтобы после МЧС target-линейка продолжала расти в новом домене.
- [ ] Решить, нужен ли для `mchs-storm-warnings` отдельный lookup регионов по человекочитаемому названию вместо ввода `regionHost` вручную.
...
- Следующий продуктовый шаг: выбрать восьмой российский read-only target-skill и зафиксировать его минимальный MVP в `docs/sources.md` и `TODO.md`.
