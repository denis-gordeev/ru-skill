...
- Выявлены и устранены гибридные термины в сводках changeset (9 файлов): `target-навык` → `целевой навык`, `target-пакет` → `целевой пакет`, `legacy-пакеты` → `устаревшие пакеты`, `на фикстурах` → `на эталонных данных`, `skill-документацией` → `документацией навыка`, `SSR-страницы` → `страницы с серверной отрисовкой (SSR)`, `серверно отрендеренные` → `сгенерированные на сервере`, `HTML-тесты` → `тесты HTML`.
- Выявлены и устранены гибридные термины в `docs/roadmap.md` (3 замены): `серверно отрендеренных/ые` → `сгенерированных/ые на сервере`, `User-facing guides` → `Пользовательские руководства`.
- `release-please` обёрнут в обратные кавычки в `python-packages/README.md`.

## Выполнено в этом раунде (раунд 88)

- [x] `.changeset/osm-nearby-add.md`: `target-навык` → `целевой навык`, `legacy-пакеты` → `устаревшие пакеты`.
- [x] `.changeset/rpl-results.md`: `target-навык` → `целевой навык`.
- [x] `.changeset/pravo-documents.md`: `target-пакет` → `целевой пакет`.
- [x] `.changeset/kinopoisk-search.md`: `target-навык` → `целевой навык`.
- [x] `.changeset/moex-shares.md`: `на фикстурах` → `на эталонных данных`, `skill-документацией` → `документацией навыка`.
- [x] `.changeset/fair-steaks-pretend.md`: `на фикстурах` → `на эталонных данных`.
- [x] `.changeset/zoon-nearby-add.md`: `SSR-страницы` → `страницы с серверной отрисовкой (SSR)`.
- [x] `.changeset/yandex-market-search.md`: `серверно отрендеренные` → `сгенерированные на сервере`.
- [x] `.changeset/postcalc-postcodes.md`: `HTML-тесты` → `тесты HTML`.
- [x] `docs/roadmap.md`: `серверно отрендеренных` → `сгенерированных на сервере` (2 вхождения), `User-facing guides` → `Пользовательские руководства`.
- [x] `python-packages/README.md`: `release-please` обёрнут в обратные кавычки.
...
- [x] `AGENTS.md`: `Политика прокси` → `Политика посредника`, `авторизации прокси` → `авторизации посредника`, `Интерфейсы прокси` → `Интерфейсы посредника`, `кэширование` → `буферизация`.
...
- Рассмотрена русификация YAML-ключей `metadata`, `locale`, `phase` во вступительных метаданных SKILL.md — признана небезопасной: ключи `name` и `description` используются в `validate-skills.sh` и `skill-docs.test.js`, а внешняя CLI `skills` также ожидает английские ключи. Рекомендовано не русифицировать.
...
- Устранён английский жаргон в docs/features/osm-nearby.md: `free/no-key` → `бесплатное решение без ключа API`, `sparse` → `неполным`.
...
- Переведён `free/no-key` → `бесплатный источник без ключа API` в docs/sources.md.
...
- [x] Русифицированы граница/продукт формулировки в 6 legacy package README без изменения code identifiers и имён API.
...
- `README.md`, `docs/roadmap.md` и `TODO.md` синхронизированы: оставшаяся корейская пользовательская копия в исходном коде и документах существенно сокращена; обусловленный доменом корейский (параметры API, названия мест, эталонные данные) сохранён корректно.
...
- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился - двигать репозиторий в сторону российских и русскоязычных сценариев, не расширяя устаревшее наследие как основной продукт.
- Этот раунд закрывает открытый вопрос Веха 5: отдельный железнодорожный навык-перенаправление поверх `yandex-rasp` не даёт новой устойчивой функции API и не должен открываться как целевой пакет.
...
- Open PR: автоматическая проверка недоступна без `gh auth login`, поэтому в этом раунде перечень запросов на слияние не подтверждён.
...
- Исследование 14-го источника (брокер/инвестиции): закрыто как избыточное — MOEX ISS уже покрыт через `moex-shares`, брокерские API требуют авторизации.
...
- [x] Подготовлены на основе эталонных данных тесты JSON для Overpass API ответов с московскими заведениями.
...
- [x] Подготовлены на основе эталонных данных тесты JSON для `stations_list`, `schedule` и `search` ответов API Яндекс.Расписаний.
...
- [x] Подготовить первый российский или русскоязычный навык поверх публичного API или официального веб-интерфейса, чтобы репозиторий перестал быть только устаревшей обёрткой над `k-skill`.
...
- [x] Проверить Zoon для замены поиска поблизости — **подтверждено**: SSR/интерфейс HTML, category + city pages, без антибота, без API keys, структурированный HTML с названиями/адресами/рейтингами/телефонами.
...
- Замены `seoul-subway-arrival` и `toss-securities` закрыты как нежизнеспособные через публичные бесплатные API — устаревшие пакеты останутся без прямых российских аналогов.
- Поиск поблизости теперь покрыт двумя источниками: `osm-nearby` (базовый, бесплатный без ключа API) и `zoon-nearby` (дополнительный, с рейтингами и контактами).
- Устаревшие пакеты с корейским контекстом сохранены как обратно совместимые, не продвигаются в документации, маркированы как `Legacy` в таблицах.
