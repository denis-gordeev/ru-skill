...
- Выявлены и устранены английские ячейки таблицы в `docs/brand-inventory.md` (4 замены): `Workspace package` → `Пакет рабочего пространства`, `npm name` → `имя npm`, `Secrets path` → `Путь к секретам`, `Env prefix` → `Префикс переменных окружения`.
- Выявлен и устранён английский жаргон в `docs/brand-inventory.md`: `capabilities` → `возможности`, `User-Agent` обёрнут в обратные кавычки.
- Выявлены и устранены английские фразы в `AGENTS.md` (2 замены): `npm workspaces` → `` `npm workspaces` `` (обёрнуто в обратные кавычки), `Version Packages` → `Version Packages (версии пакетов)` (добавлен русский глоссарий).
- Обёрнуты в обратные кавычки голые английские идентификаторы полей API в `docs/sources.md` и `packages/rpl-results/README.md` (2 файла).

## Выполнено в этом раунде (раунд 87)

- [x] `docs/brand-inventory.md`: `Workspace package` → `Пакет рабочего пространства`, `npm name` → `имя npm`.
- [x] `docs/brand-inventory.md`: `Secrets path` → `Путь к секретам`.
- [x] `docs/brand-inventory.md`: `Env prefix` → `Префикс переменных окружения`.
- [x] `docs/brand-inventory.md`: `capabilities` → `возможности`, `User-Agent` обёрнут в обратные кавычки.
- [x] `AGENTS.md`: `npm workspaces` → `` `npm workspaces` ``, `Version Packages` → `Version Packages (версии пакетов)`.
- [x] `docs/sources.md`: идентификаторы полей API обёрнуты в обратные кавычки.
- [x] `packages/rpl-results/README.md`: идентификаторы полей API обёрнуты в обратные кавычки.
