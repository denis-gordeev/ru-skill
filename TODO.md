...
- Периодический аудит выявил: возвраты жаргонизмов в глубоких исторических секциях `TODO.md` и `docs/roadmap.md` — `CLI` без раскрытия (2 вхождения), `валидацию` (1 вхождение), `Open PR` (1 вхождение), `аккаунт` (1 вхождение), `Overpass API` как самостоятельный термин (4 вхождения), `free/no-key` (2 вхождения), `Legacy` без русского пояснения (1 вхождение), `PR` и `backlog` без русского раскрытия (1 вхождение); все устранены. В `.changeset/osm-nearby-add.md` — `Overpass API` (1 вхождение); устранено.
- Остальные ключевые жаргонизмы не вернулись в интерфейсной документации.

## Выполнено в этом раунде (раунд 122)

- [x] `TODO.md`: `внешняя CLI` → `внешний интерфейс командной строки (CLI)` (1 вхождение, строка 913).
- [x] `TODO.md`: `валидацию` → `проверку корректности` (1 вхождение, строка 1510).
- [x] `TODO.md`: `Open PR` → `Открытые запросы на слияние` (1 вхождение, строка 1950).
- [x] `TODO.md`: `Overpass API (OSM) выбран как лучший free/no-key вариант` → `программный интерфейс Overpass (OSM) выбран как лучший бесплатный вариант без ключа` (1 вхождение, строка 1984).
- [x] `TODO.md`: `T-Invest требует аккаунт` → `T-Invest требует учётную запись` (1 вхождение, строка 1986).
- [x] `TODO.md`: `Overpass API ответов` → `ответов программного интерфейса Overpass` (1 вхождение, строка 1988).
- [x] `TODO.md`: `Overpass API (OpenStreetMap) как free/no-key вариант` → `программный интерфейс Overpass (OpenStreetMap) как бесплатный вариант без ключа` (1 вхождение, строка 2078).
- [x] `TODO.md`: `через Overpass API` → `через программный интерфейс Overpass` (1 вхождение, строка 2079).
- [x] `TODO.md`: `маркированы как \`Legacy\`` → `маркированы как устаревшие (\`Legacy\`)` (1 вхождение, строка 2105).
- [x] `TODO.md`: `PR недоступны` + `backlog управляется` + `прямые PR` → `запросы на слияние недоступны` + `перечень задач управляется` + `прямые запросы на слияние` (1 вхождение, строка 2124).
- [x] `docs/roadmap.md`: `внешнюю CLI` → `внешний интерфейс командной строки (CLI)` (1 вхождение, строка 277).
- [x] `.changeset/osm-nearby-add.md`: `через Overpass API OpenStreetMap` → `через программный интерфейс Overpass OpenStreetMap` (1 вхождение, строка 5).
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; жаргонизмы в глубоких исторических секциях и changeset устранены.
