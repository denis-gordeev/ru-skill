...
- Периодический аудит выявил: `OIDC` без раскрытия в `AGENTS.md` (1 вхождение: → `OIDC (OpenID Connect)`), `dotenv` без русского пояснения в `docs/security-and-secrets.md` (4 вхождения: → `файла окружения (dotenv)` / `формат файла окружения (dotenv)` / `файле окружения (dotenv)` / `файлом окружения (dotenv)`) и `docs/setup.md` (1 вхождение: → `формате файла окружения (dotenv)`); все устранены.
- Остальные ключевые жаргонизмы не вернулись в интерфейсной документации.

## Выполнено в этом раунде (раунд 116)

- [x] `AGENTS.md`: `через OIDC для npm и PyPI` → `через OIDC (OpenID Connect) для npm и PyPI` (1 вхождение).
- [x] `docs/security-and-secrets.md`: `в формате dotenv` → `в формате файла окружения (dotenv)` (1 вхождение), `простой dotenv` → `простой формат файла окружения (dotenv)` (1 вхождение), `в том же файле dotenv` → `в том же файле окружения (dotenv)` (1 вхождение), `обычным файлом dotenv` → `обычным файлом окружения (dotenv)` (1 вхождение).
- [x] `docs/setup.md`: `в формате dotenv` → `в формате файла окружения (dotenv)` (1 вхождение).
- [x] Периодический аудит интерфейсной документации: ключевые жаргонизмы не вернулись; `OIDC` без раскрытия и `dotenv` без русского пояснения устранены.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда и следующим планом.

## Новые пункты плана (перенесены из раунда 115 — выполнены в раунде 116)
