...
## Статус на 2026-08-20 (раунд 131)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Завершён оставшийся после раунда 124 слой замены `URL` как самостоятельного существительного: сообщения об ошибках имитированных запросов в тестах 7 целевых пакетов теперь используют слово `адрес`.
- Документная регрессия расширена общей проверкой всех 7 затронутых пакетов, чтобы формулировка `Неожиданный имитированный URL` не вернулась.

## Выполнено в этом раунде (раунд 131)

- [x] В тестах `hh-vacancies`, `kinopoisk-search`, `mchs-storm-warnings`, `moex-shares`, `postcalc-postcodes`, `pravo-documents` и `stoloto-lotto` заменено `Неожиданный имитированный URL` на `Неожиданный имитированный адрес`.
- [x] В `scripts/skill-docs.test.js` добавлена единая регрессионная проверка сообщений об ошибках имитированных запросов для всех 7 пакетов.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы со статусом раунда 131.

## Новые пункты плана

- [ ] Проверить пользовательские ошибки целевых пакетов, где имена параметров (`page`, `perPage`, `pageSize`, `secId`) выводятся без русского пояснения; выбрать единый формат с сохранением точного программного идентификатора в обратных кавычках.
- [ ] Расширить документную регрессию на согласованный формат имён параметров в пользовательских ошибках целевых пакетов.
- [ ] Проверить оставшиеся пользовательские сообщения переходного `k-skill-proxy` и устаревших пакетов на необъяснённые английские идентификаторы, не меняя публичные программные контракты.
