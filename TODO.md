...
- [x] Выбрать восьмой российский read-only источник, чтобы после МЧС target-линейка продолжала расти в новом домене.
- [x] Решить, нужен ли для `mchs-storm-warnings` отдельный lookup регионов по человекочитаемому названию вместо ввода `regionHost` вручную.
...
- [ ] Выбрать девятый российский read-only источник, чтобы после pravo.gov.ru target-линейка охватила ещё один практический домен (например, транспорт или городские сервисы).
...
- Следующий продуктовый шаг: начать матрицу замены legacy-пакетов (Milestone 4) или выбрать девятый российский источник в новом домене (транспорт/городские сервисы).
