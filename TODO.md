...
- [ ] Проверить `docs/sources.md` и `docs/brand-inventory.md` на оставшиеся вхождения `поверхность` как калка.
- [ ] Расширить документную регрессию на отсутствие `поверхность` (в значении surface) в пользовательских интерфейсных документах.
