...
- Устранён следующий точечный слой английского жаргона в `package README` и корневом `README.md`: `серверно отрендеренные страницы (SSR)` → `страницы с серверным рендерингом (SSR)` и `Серверный рендеринг (SSR)` (`packages/zoon-nearby/README.md`), `HTML-парсинг` → `HTML-разбор` (`packages/kleague-results/README.md`), `адаптеры` → `модули-посредники` (`packages/k-skill-proxy/README.md`), `Данные парсятся` → `Данные разбираются` (`packages/rpl-results/README.md`), а подписи ссылок на `blue-ribbon-nearby` и `kakao-bar-nearby` в корневой таблице README стали более русскоязычными.
- Документная регрессия расширена: добавлены проверки раунда 74 на отсутствие `HTML-парсинг`, `Данные парсятся`, `адаптеры` в `packages/k-skill-proxy/README.md`, а также на более русскоязычные подписи ссылок `Blue Ribbon nearby` / `Kakao bar nearby` в корневом `README.md`.

## Выполнено в этом раунде (раунд 74)

- [x] `packages/zoon-nearby/README.md`: описание источника и блок преимуществ переведены на формулировку `серверный рендеринг (SSR)`; `парсится` → `разбирается`.
- [x] `packages/kleague-results/README.md`: `HTML-парсинг` → `HTML-разбор` (2 вхождения).
- [x] `packages/k-skill-proxy/README.md`: `адаптеры` → `модули-посредники` (2 вхождения).
- [x] `packages/rpl-results/README.md`: `Данные парсятся` → `Данные разбираются`.
- [x] `README.md`: ссылки на `blue-ribbon-nearby` и `kakao-bar-nearby` в таблице навыков переведены на более русскоязычные подписи; раздел `Что уже сделано по миграции` дополнен итогом этого раунда.
- [x] `scripts/skill-docs.test.js`: добавлены регрессии раунда 74 под этот слой package README и корневых ссылок README.
- [x] `README.md`, `TODO.md` и `docs/roadmap.md` синхронизированы с новым статусом и следующим перечнем задач.

## Новые пункты плана

- [ ] Продолжить аудит оставшихся `package README` на гибриды вокруг `CLI`, `парсинг/разбор`, `серверный рендеринг (SSR)` и других технических описаний.
- [ ] Провести точечный аудит корневых `SKILL.md`, которые ещё содержат англоязычные технические ярлыки в прозе, не затрагивая кодовые идентификаторы и YAML-ключи.
- [ ] Проверить `scripts/*.js`, `scripts/*.sh` и комментарии в инфраструктурных файлах на следующий слой пользовательских гибридов.
