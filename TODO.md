...
## Статус на 2026-07-20 (раунд 101)

- `AUTOWORK_INSTRUCTIONS.md`: приоритет не изменился — переделать всё под российские / русскоязычные реалии.
- Периодический аудит интерфейсной документации выявил возврат `авторизованных` (форма от `авторизация`) в 3 файлах: `toss-securities/SKILL.md`, `packages/toss-securities/README.md`, `docs/features/toss-securities.md`. Все заменены на `с проверкой подлинности`.
- Выявлены и устранены несогласованности `браузер` → `обозреватель` в 4 файлах: `toss-securities/SKILL.md`, `hwp/SKILL.md`, `docs/features/moex-shares.md`, `docs/features/hwp.md`.
- Выявлена и устранена тавтология `интерфейсы API` → `программные интерфейсы` в `docs/features/kleague-results.md`.
- Унифицированы голые английские термины в интерфейсной документации: `JSON`/`HTML`/`DOM`/`Fastify`/`PM2`/`JavaScript`/`npm`/`curl`/`base64` обёрнуты в обратные кавычки или дополнены русскими пояснениями (`в формате JSON`, `разметка HTML`, `объектной модели документа (DOM)`) в 13 файлах.
- Унифицирован порядок слов в YAML-описаниях 3 SKILL.md: `пакет NAME npm` → `пакет NAME из реестра npm`.
- 11 ключевых жаргонизмов (`релиз`, `прокси`, `кэш`, `эндпоинт`, `парсинг`, `токен`, `аутентификация`, `хост`, `аккаунт`, `логин`, `авторизация`) в интерфейсной документации не обнаружены.

## Выполнено в этом раунде (раунд 101)

- [x] `toss-securities/SKILL.md`: `авторизованных` → `с проверкой подлинности`, `Сеанс браузера` → `Сеанс обозревателя`.
- [x] `packages/toss-securities/README.md`: `авторизованных` → `с проверкой подлинности`.
- [x] `docs/features/toss-securities.md`: `авторизованных` → `с проверкой подлинности`.
- [x] `hwp/SKILL.md`: `в браузере` → `в обозревателе`.
- [x] `docs/features/moex-shares.md`: `без браузера` → `без обозревателя`.
- [x] `docs/features/hwp.md`: `в браузере` → `в обозревателе`, `base64` → `` `base64` ``, `Для JSON:` → `Для формата JSON:`.
- [x] `docs/features/kleague-results.md`: `официальный JSON и официальные интерфейсы API` → `данные в формате JSON и официальные программные интерфейсы`.
- [x] `docs/features/k-skill-proxy.md`: `на Fastify` → `` на `Fastify` ``, `через PM2` → `` через `PM2` ``.
- [x] `docs/features/kakao-bar-nearby.md`: `JSON может отвечать` → `в формате JSON может отвечать`.
- [x] `docs/features/delivery-tracking.md`: `конечную точку JSON` → `конечную точку в формате JSON`, `к HTML, а не к JSON` → `к формату HTML, а не к формату JSON`, `пакеты npm` → `` пакеты `npm` ``.
- [x] `docs/features/lotto-results.md`: `из официального ответа JSON` → `из официального ответа в формате JSON`.
- [x] `docs/features/ktx-booking.md`: `из ответа JSON` → `из ответа в формате JSON`, `приходят в JSON` → `приходят в формате JSON`.
- [x] `docs/features/rpl-results.md`: `программ разбора DOM` → `программ разбора объектной модели документа (DOM)`, `JavaScript` → `` `JavaScript` ``.
- [x] `docs/features/zipcode-search.md`: `страницу HTML` → `страницу в формате HTML`, `параметрами curl` → `` параметрами `curl` ``.
- [x] `docs/features/yandex-market-search.md`: `интерфейс HTML Яндекс Маркета` → `интерфейс разметки HTML Яндекс Маркета`.
- [x] `docs/features/postcalc-postcodes.md`: `на HTML на основе` → `на разметку HTML на основе`.
- [x] `kbo-results/SKILL.md`: `пакет kbo-game npm` → `пакет kbo-game из реестра npm`.
- [x] `lotto-results/SKILL.md`: `пакет k-lotto npm` → `пакет k-lotto из реестра npm`.
- [x] `kleague-results/SKILL.md`: `конечную точку JSON и пакет kleague-results npm` → `конечную точку в формате JSON и пакет kleague-results из реестра npm`.
- [x] `scripts/skill-docs.test.js`: регрессионные тесты обновлены под новую терминологию.
- [x] Периодический аудит интерфейсной документации: 11 ключевых жаргонизмов не вернулись; `браузер`, тавтология `интерфейсы API` и голые английские термины устранены.
...
- [ ] Рассмотреть политику замены `API` → `программный интерфейс` в связном тексте интерфейсной документации (аудит выявил ~43 вхождения `API` как самостоятельного существительного в 14 файлах docs/features/*.md; тавтология `интерфейсы API` уже устранена).
